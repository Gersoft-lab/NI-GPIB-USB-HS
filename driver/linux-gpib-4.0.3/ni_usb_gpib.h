...
typedef struct
{
	 ...
	 * firmware and drains the bulk-in pipe to resynchronize. */
	int pipe_dirty;
	/* the firmware IBGTS request (sent after each parallel poll to release
	 * the IDY state) clears the firmware's addressing state; the last
	 * addressing command sequence seen is replayed before the next write. */
	int needs_readdress;
	uint8_t last_addressing[16];
