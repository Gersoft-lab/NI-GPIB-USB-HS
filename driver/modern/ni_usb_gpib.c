...
static void ni_usb_resync_pipe(struct ni_usb_priv *ni_priv)
{
	...
	dev_err(&usb_dev->dev, "pipe resync: drained %i stale byte(s)\n", total);
}
					       ...
					       int interruptible)
{
	...
	*actual_data_length = 0;
	...
	if (context->timed_out) {
		 ...
		 */
		ni_usb_stop(ni_priv);
		if (!wait_for_completion_timeout(&context->complete,
	...
	*actual_data_length = ni_priv->bulk_urb->actual_length;
	...
	mutex_unlock(&ni_priv->bulk_transfer_lock);
	if (urb_killed) {
		/* scoop the orphaned response if it still arrives, so the
		 * pipe stays usable */
		u8 *scratch = kmalloc(0x200, GFP_KERNEL);
		...
		if (scratch) {
			...
			for (n = 0; n < 4; n++) {
				got = 0;
				r = usb_bulk_msg(usb_dev, in_pipe, scratch,
						 0x200, &got, 300);
				if (r || got <= 0)
					break;
				dev_err(&usb_dev->dev, "drained %i byte(s) of late response\n",
					got);
			}
			kfree(scratch);
		}
				     ...
				     int *actual_bytes_read)
{
	...
	if (*actual_bytes_read > j) {
		 ...
		 * actually parsed out of the data blocks. */
		pr_err("bug: discarded data. actual_bytes_read=%i, j=%i (firmware garbage count? clamping)\n",
		       *actual_bytes_read, j);
		*actual_bytes_read = j;
	}
	for (k = 0; k < 2; k++)
				  ...
				  unsigned int *ibsta)
{
	...
	if (status.id != NIUSB_REG_WRITE_ID) {
		...
		ni_priv->pipe_dirty = 1;	/* response shifted */
		return -EIO;
	}
			...
			int send_eoi, size_t *bytes_written)
{
	 ...
	 * this is what makes streamed (EOI-less) writes usable at full speed.
	 */
	...
	if (send_eoi == 0 && retval && retval != -ERESTARTSYS && usb_bytes_read == 0) {
		...
		dev_info(&usb_dev->dev,
			 "quirk 6: no ack for EOI-less write, emulating success from now on\n");
	...
	if ((size_t)status.count > length) {
		dev_err(&usb_dev->dev, "bogus status block: id=0x%x error=%i count=%i length=%i (pipe desync?)\n",
			status.id, status.error_code, status.count, (int)length);
		ni_priv->pipe_dirty = 1;
		*bytes_written = 0;
		return -EIO;
	}
...
static int ni_usb_go_to_standby(struct gpib_board *board)
{
	 ...
	 * after every send_setup, which broke all addressed writes.  The
	 * firmware's own read/write requests manage ATN themselves, so
	 * skipping the explicit gts is safe.  Chip-level AUX_GTS pokes are
	 * NOT an option either (any out-of-band AUXMR write corrupts the
	 * engine the same way). */
	return 0;
...
static int ni_usb_parallel_poll(struct gpib_board *board, u8 *result)
{
	...
	/* serialize like every other request/response pair (this function
	 * was the only one left without the lock) */
	mutex_lock(&ni_priv->addressed_transfer_lock);
	 ...
	 * replug (recent GPIB-USB-HS firmware quirk). */
	{
		...
		if (scratch) {
			for (n = 0; n < 8; n++) {
				...
				if (drained <= 0)
					break;
				dev_err(&usb_dev->dev, "attach: drained %i stale byte(s) from bulk-in pipe\n",
					drained);
			}
			kfree(scratch);
		}
